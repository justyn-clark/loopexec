...
- `loopexec doctor`
  ...
  - Flags: `--check "<cmd>"`, `--runs N`, `--max-flake-rate R`, `--workdir`, plus isolation preflight `--bind-model-home` (a `$HOME/.model-home` credential mount fails closed -> `credential_scope_invalid`, 13), `--exec-network <policy>` (must be `none` -> else `isolation_unsatisfiable`, 30), and `--mutate-cmd "<cmd>"` (an operator command that plants a mutation in the changed code; the `--check` MUST then turn red, else `check_inadequate`, 15). The canary runs in an isolated copy of the workdir, so the real tree is never mutated; it is skipped when the check is not deterministic.
