...
func newDoctorCmd() *cobra.Command {
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if bindModelHome {
				dr.Checks = append(dr.Checks, doctorCheck{Name: "isolation-credentials", Status: "fail",
					Detail: "a $HOME/.model-home bind-mount exposes a long-lived key; mint a per-run scoped, spend-capped key instead"})
				if haltReason == "" {
	...
	cmd.Flags().StringVar(&workdir, "workdir", "", "Directory to run the check in (default: current directory)")
	cmd.Flags().BoolVar(&bindModelHome, "bind-model-home", false, "Declare a $HOME/.model-home credential bind-mount (fails the isolation preflight)")
